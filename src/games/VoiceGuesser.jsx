...
import { 
  Play, Pause, ArrowRight, Check, X, Sparkles, 
  Volume2, VolumeX, Lock, Eye
} from 'lucide-react'
...
export default function VoiceGuesser({ soundEnabled, onBack }) {
  ...
  // Practice Mode default (โหมดเดียว)
  ...
  const playPrimaryAudio = () => {
    ...
    audio.play()
      ...
      .catch(err => {
        console.error("Failed to play primary audio:", err)
        setIsAudioLoading(false)
        setPlaybackError(true)
      })
  }
    ...
    if (hintAudioRef.current) {
      if (isHintPlaying) {
        ...
        hintAudioRef.current.play()
          ...
          .catch(err => {
            console.error("Failed to play secondary audio hint:", err)
            setIsHintAudioLoading(false)
          })
      }
      return
    ...
    >
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .media-disc-viewport {
          ...
          box-shadow: 0 4px 25px rgba(0, 0, 0, 0.4), inset 0 0 10px rgba(255,255,255,0.05);
          display: flex;
          align-items: center;
        ...
        .waveform-bar-item.animating {
          ...
          animation: dance-wave 1.2s ease-in-out infinite alternate;
        }
        .waveform-bar-item:nth-child(1) { animation-delay: 0.1s; }
        .waveform-bar-item:nth-child(2) { animation-delay: 0.25s; }
        .waveform-bar-item:nth-child(3) { animation-delay: 0.05s; }
        .waveform-bar-item:nth-child(4) { animation-delay: 0.4s; }
        .waveform-bar-item:nth-child(5) { animation-delay: 0.2s; }
        .waveform-bar-item:nth-child(6) { animation-delay: 0.35s; }
        .waveform-bar-item:nth-child(7) { animation-delay: 0.15s; }
        .waveform-bar-item:nth-child(8) { animation-delay: 0.3s; }
        .waveform-bar-item:nth-child(9) { animation-delay: 0.08s; }
        .waveform-bar-item:nth-child(10) { animation-delay: 0.22s; }
        ...
        .voice-volume-input::-webkit-slider-thumb {
          -webkit-appearance: none;
          width: 10px;
          height: 10px;
          border-radius: 50%;
        ...
        .inline-audio-play-btn {
          ...
          align-items: center;
          gap: 6px;
          background: rgba(76, 154, 224, 0.1);
          border: 1px solid rgba(76, 154, 224, 0.2);
        ...
        .inline-audio-play-btn:hover {
          background: rgba(76, 154, 224, 0.25);
          ...
          <div className="halo-gameplay-header">
            ...
            <div className="gameplay-hud-stats">
              ...
              <div className="hud-stat-box timer">
                ...
                <div className="hud-val" style={{ color: 'var(--color-accent)' }}>{currentTarget.voiceLines.length}</div>
              </div>
            </div>
          </div>
                  ...
                  <AnimatePresence mode="wait">
                    {solved ? (
                      <motion.div 
                        ...
                        className="media-disc-viewport solved-avatar-glow" 
                      ...
                      <motion.div 
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0, scale: 0.8 }}
                        className={`media-disc-viewport ${isPlaying ? 'media-disc-spin' : ''}`}
                      >
                        <div className="absolute inset-2 border border-slate-700/50 rounded-full border-dashed"></div>
                        <Volume2 className={`w-12 h-12 ${isPlaying ? 'animate-pulse' : ''}`} style={{ color: 'var(--color-accent)', filter: 'drop-shadow(0 0 8px rgba(76, 154, 224, 0.6))' }} />
                      </motion.div>
                    )}
                  </AnimatePresence>
            ...
            <div className="halo-guesser-section">
              ...
              <AnimatePresence mode="wait">
                {!solved ? (
                  ...
                  >
                    <h4 className="guesser-input-title" style={{ color: 'var(--color-accent)' }}>ป้อนชื่อตัวละครนักเรียนที่พูดประโยคข้างต้น:</h4>
