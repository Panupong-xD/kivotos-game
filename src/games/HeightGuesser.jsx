...
import { Trophy, Play, RotateCcw, ArrowRight, Check, X, ShieldAlert, ArrowUp, ArrowDown } from 'lucide-react'
...
export default function HeightGuesser({ soundEnabled, onBack, setCustomBackAction }) {
  ...
  const [revealedHeight, setRevealedHeight] = useState(null) // State to count up right student height
  const [isRevealing, setIsRevealing] = useState(false)
  const [guessResult, setGuessResult] = useState(null) // 'correct' or 'incorrect'
  const [userChoice, setUserChoice] = useState(null) // 'taller' or 'shorter'
    ...
    async function loadData() {
      ...
      try {
        ...
        for (const key in data) {
          const char = data[key]
          // Filter characters that are students (TemplateType === 'Character')
          // and have valid numeric heights
          const heightVal = parseInt(char.Height)
          if (
  ...
  // Sets up a new pair of students
  // If prevRight is provided, it becomes the new left student
  const setupNewPair = (list, prevRight = null, currentScore = null) => {
    ...
    // Filter potential challenger (Right) to have a DIFFERENT height
    const availablePool = pool.filter(s => s.key !== left.key && s.height !== left.height)
    if (availablePool.length === 0) {
      // Fallback in case of lack of students
  ...
  const handleGuess = (choice) => {
    ...
    const isTaller = rightStudent.height > leftStudent.height
    const isCorrect = (choice === 'taller' && isTaller) || (choice === 'shorter' && !isTaller)

    // Run custom number ticker animation for right student's height
    const startVal = 100
    const endVal = rightStudent.height
    ...
    const ticker = setInterval(() => {
      ...
      const currentHeight = Math.round(startVal + (endVal - startVal) * progress)
      setRevealedHeight(currentHeight)

      if (currentStep >= steps) {
        clearInterval(ticker)
        setRevealedHeight(endVal)
        
        // Show result overlay and play sound
        if (isCorrect) {
  ...
  return (
    <div className="height-game-layout">
      {/* Header Info */}
      <div className="height-game-header">
        <div className="height-mode-info">
          <h2>วัดส่วนสูงนักเรียน (Height Compare)</h2>
          <p>ทายว่านักเรียนทางขวา สูงกว่าหรือเตี้ยกว่านักเรียนทางซ้าย?</p>
        </div>
        <div className="height-stats-bar">
      ...
      {/* Main Game Screen */}
      <div className="height-comparison-wrapper">
        
        {/* Left Student Card: Known Height */}
        {leftStudent && (
          <div className="height-student-card left-card" style={{ backgroundImage: getSchoolGradient(leftStudent.school) }}>
            ...
            <div className="card-student-info">
              ...
              <div className="height-value-box">
                <span className="height-num">{leftStudent.height}</span>
        ...
        {/* VS Divider */}
        <div className="height-vs-badge">
          <span>VS</span>
        </div>

        {/* Right Student Card: Guess Height */}
        {rightStudent && (
          <div className="height-student-card right-card" style={{ backgroundImage: getSchoolGradient(rightStudent.school) }}>
            <div className="card-school-badge">
            ...
            <div className="card-student-info">
              ...
              <div className="height-value-box">
                <span className={`height-num ${revealedHeight !== null ? 'revealed' : 'masked'}`}>
                  {revealedHeight !== null ? revealedHeight : '?'}
                </span>
                <span className="height-unit">cm</span>
              </div>
            </div>
            ...
            {guessResult && (
              <div className={`height-reveal-overlay ${guessResult}`}>
                {guessResult === 'correct' ? (
      ...
      {/* Control Buttons (Untimed Mode) */}
      {!gameOver && (
        <div className="height-guess-controls">
          <button 
            onClick={() => handleGuess('taller')}
            disabled={isRevealing}
            className={`guess-btn taller ${userChoice === 'taller' ? 'selected' : ''}`}
          >
            <ArrowUp className="w-5 h-5" />
            <span>สูงกว่า (Taller)</span>
          </button>

          <button 
            onClick={() => handleGuess('shorter')}
            disabled={isRevealing}
            className={`guess-btn shorter ${userChoice === 'shorter' ? 'selected' : ''}`}
          >
            <ArrowDown className="w-5 h-5" />
            <span>เตี้ยกว่า (Shorter)</span>
          </button>
        </div>
      )}
      ...
      {/* Game Over Screen */}
      {gameOver && (
        <div className="height-gameover-overlay">
          <div className="height-gameover-card">
            <div className="gameover-warning-icon">
              <ShieldAlert className="w-16 h-16 text-rose-500" />
            </div>
            <h2>สิ้นสุดการท้าทาย!</h2>
            <p>ความสูงจริงของ <strong>{rightStudent?.nameEn}</strong> คือ <strong>{rightStudent?.height} cm</strong></p>
            ...
            <button onClick={handleRestart} className="height-restart-btn">
